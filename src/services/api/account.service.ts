...
import { OnError, Post, Patch, Body, Query, Header, ResponseInterceptor } from "restfit";
...
export class AccountService {
  ...
  @Post<void>("/account/email/send-reactivation")
  async sendReactivationVerificationAsync(
    @Body() payload: SendVerificationRequestType
  ): Promise<void> {
    return;
  }
  ...
  @Post<void>("/account/delete")
  async deleteAccountAsync(@Body() feedback: AccountDeletionFeedback): Promise<void> {
    return;
  }

  @Patch<void>("/account/deactivate")
  async deactivateAccountAsync(): Promise<void> {
    return;
  }

@Post("/account/export")
async exportData(): Promise<ServiceResponse> {
  return {} as ServiceResponse;
}

@Patch("/account/privacy-settings")
async updatePrivacySettings(
  @Body() body: { profilePrivacy: string }

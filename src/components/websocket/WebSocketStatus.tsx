        ...
        <div className="fixed bottom-4 right-4 bg-card border border-border rounded-lg shadow-lg p-4 max-w-sm z-50">
            <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                    <span className={`w-2 h-2 rounded-full ${overallConnected ? 'bg-success' : 'bg-destructive'} animate-pulse`} />
            ...
            <div className="space-y-1 text-xs mb-3">
                <div className="flex items-center gap-2">
                    <span className={isNotificationsConnected ? "text-success" : "text-destructive"}>
                ...
                <div className="flex items-center gap-2">
                    <span className={isImportsConnected ? "text-success" : "text-destructive"}>
                        {isImportsConnected ? "🟢" : "🔴"}
                    </span>
                    <span>Imports</span>
                </div>
                ...
                <div className="flex items-center gap-2">
                    <span className={sessionSecurityConnected ? "text-success" : "text-destructive"}>
                    ...
                    <div className="space-y-1 max-h-32 overflow-y-auto">
                        {notifications.slice(0, 3).map((notif) => (
                            <div
                                ...
                                className={`text-xs p-2 rounded cursor-pointer ${notif.isRead ? "bg-muted" : "bg-accent"
                                    }`}
                            >
                    ...
                    <div className="space-y-1 max-h-20 overflow-y-auto">
                        {recentImports.slice(0, 3).map((imp, i) => (
                            <div key={i} className="text-xs p-1 bg-muted rounded">
                                ...
                                {imp.title && (
                                    <span className="text-muted-foreground">: {imp.title}</span>
                                )}
                            </div>
                        ))}
                    </div>

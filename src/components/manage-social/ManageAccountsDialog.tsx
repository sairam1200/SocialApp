...
export function ManageAccountsDialog({
	...
	const handleStopImporting = React.useCallback(
		async (platformId: PlatformId) => {
			await apiClient.Integration.cancelImport(platformId, { confirm: true });
			const status = await apiClient.Integration.getImportStatus(platformId);
			if (
				status.status === "cancelled" ||
				status.status === "not_found" ||
				status.status === "failed"
			) {
				onImportStatusChange(platformId, "not_imported");
			}
		},
		[onImportStatusChange]
	);

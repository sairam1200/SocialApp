...
function CustomizeStep({
	...
	const effectiveValues = {
		...
		boardId: activeOverride?.boardId ?? "",
			...
			<div>
				...
				<div className="grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-3 text-black-default items-start">
					<div className="min-w-0 space-y-3 overflow-y-auto pr-2 max-h-[450px]">
						...
						{activePlatformId === "pinterest" && (
							<div className="space-y-2">
								<label className="font-bold text-sm block">Pinterest Board <span className="text-destructive">*</span></label>
								<Select
									placeholder="Select a board"
									value={effectiveValues.boardId}
									onValueChange={(value) => formik.setFieldValue(`platformOverrides.${activePlatformId}.boardId`, value)}
									options={Array.from(new Map((PinterestContents ?? []).filter((item) => item.boardId).map((item) => [item.boardId!, { value: item.boardId!, label: item.boardName || item.boardId! }])).values())}
									helperText={PinterestContents?.length ? undefined : "No Pinterest board was found. Create a board in Pinterest, then reconnect or refresh."}
								/>
							</div>
						)}
						...
						{isYoutube && (
							<div className="space-y-2">
								...
								<div className="flex items-center gap-3">
									...
									/>
					<span className="text-xs text-gray-neutral">Max 2 MB. Recommended: 1280×720, JPEG or PNG.</span>
								</div>
							</div>
						)}

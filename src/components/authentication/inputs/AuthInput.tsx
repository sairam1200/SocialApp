...
const AuthInput: FC<IInputProps> = ({
  ...
  return (
    <div className="mt-4">
      <label
        htmlFor={name}
        className={`block select-none ${labelClassName || "text-sm font-medium text-[#595959] mb-2"}`}
      >
        {label}
      ...
      <div className="relative">
        ...
        <input
          ...
          style={style}
          className={cn(
            "w-full text-base font-normal pr-3 py-2 bg-[#FFFFFF] border rounded-lg focus:outline-none focus:ring-1 focus:border-transparent",
            error
              ? "focus:ring-red-700 border-red-700"
              : "focus:ring-[#6400BF] border-[#A288FF]",
            placeholderIcon
              ? "pl-9"
              : "pl-4"
          )}

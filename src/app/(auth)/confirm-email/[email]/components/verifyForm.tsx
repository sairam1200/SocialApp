...
export default function VerifyForm({ email, onSuccess }: VerifyFormProps) {
  ...
  const renderMessageButton = () => (
    <Button
      ...
      className={`bg-none text-[#241357] text-xs font-bold underline-offset-0 shadow-none py-0 px-0 ${isResendLoading ? "no-underline text-[#241357] shadow-none text-xs relative top-1 left-[-9] py-0 px-0 disabled:opacity-70" : ""}`}
  ...
  return (
    <div className="max-w-[359px] mx-auto py-6">
      ...
      {secondsLeft > 0 && !hideTimer && (
        <div className="text-center mt-4 mb-2 text-xs text-[#241357]">{formatTime(secondsLeft)}</div>
      )}

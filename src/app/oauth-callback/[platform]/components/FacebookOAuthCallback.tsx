...
import type { FacebookOAuthTokenResponseType } from "@/types/auth/login.type";
...
async function storePlatformTokens(
  facebookResponse: FacebookOAuthTokenResponseType
): Promise<void> {
  if (facebookResponse?.facebookAccessToken) {
    const maxAge = facebookResponse.facebookAccessTokenExpiresIn || DEFAULT_TOKEN_EXPIRY;
    await setCookie(COOKIE_NAMES.FACEBOOK_ACCESS_TOKEN, facebookResponse.facebookAccessToken, {
      maxAge,
    });
  }
}
...
async function handleAuthSuccess(
  response: FacebookOAuthTokenResponseType
): Promise<void> {
  ...
  useEffect(() => {
    ...
    async function processCallback() {
      ...
      try {
        ...
        const result = await apiClient.Token.callbackAsync<FacebookOAuthTokenResponseType>(
          "facebook",
          code,
          state,
          redirectUrl

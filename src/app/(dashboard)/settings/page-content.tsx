...
import CustomizeGaddrDialog from "./components/CustomizeGaddrDialog";
		...
		<div className="space-y-10">
			...
			<div className="rounded-[20px] overflow-hidden shadow shadow-[#F4D7FF]">
				...
				<div className="relative w-full h-[120px] rounded-xl mb-10">
						<Image src={data?.coverPhoto || "/images/default-bg.svg"} alt="Header background" fill className="object-cover" priority />
					...
					<div className="absolute left-10 -bottom-9">
						<div
									className="relative w-14 h-14 rounded-full border-4 border-white shadow-md overflow-hidden group"
						>
			...
			<CustomizeGaddrDialog
				open={openCustomizeGaddrDialog}
				onClose={() => setOpenCustomizeGaddrDialog(false)}
				onCoverPhotoUploaded={(coverPhoto) => {
					setData((current) => current ? { ...current, coverPhoto } : current);
				}}
			/>
						</div>

				...
				onConfirm={async () => {
					...
					try {
						if (connectionType === "manual" || connectionType === "custom") {
							...
							try {
								const rawResult = await apiClient.Integration.disconnect(
									platformName.trim().toLowerCase()
								);
								// `wrapResponses` is enabled globally, so support both the
								// direct API shape and `{ data: ... }` without treating a
								// successful YouTube disconnect as a failure.
								const result = unwrapResponse(rawResult);

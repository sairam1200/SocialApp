...
- [src/types/search.types.ts](src/types/search.types.ts) - TypeScript interfaces for search API
  - `FlatSearchResponse` - Flat API response (`items`, `pagination`, `facets`)
  - `FlatSearchItem` - Single result item (`type`: content/profile/project/job)
  - `SearchEntityType` - Wire entity type (content | profile | project | job)
  - `SearchResult` - Normalized UI result item
...
export function MySearchComponent() {
  ...
  const searchState = useSearch({ debounceMs: 300 });

  const handleSearch = (term: string) => {
    searchState.debouncedSearch(term);
  };
...
When `type` is set, the backend filters to that entity type and pagination applies to it alone. When omitted, `items` are the top-ranked mix across all types and `facets` carries the per-type totals.
...
Mock data is only used for trending content in development:

- **Trending Items**: 5 predefined trending topics
- **Realistic Data**: Includes growth metrics and platform lists

Search results always hit the real backend; there is no mock search path.
...
interface UseSearchOptions {
  ...
  enabled?: boolean;     // Allow the query to run
  platforms?: string[];  // Restrict results to platforms
}
```
...
- Confirm the backend flat `/search` endpoint responds
- Test the query directly in the network tab

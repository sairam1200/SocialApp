...
- `staging` is the integration branch and MUST NOT receive direct feature commits.
- Every task MUST start from the latest `staging`.
- Before coding, you MUST sync your local `staging` with remote (`checkout develop` -> `pull`).
- Feature work MUST be done on a new branch created from `staging`.
...
5. push branch and open PR into `staging`
...
- etag signed URl: for media upload
...
| `NEXT_PUBLIC_WEBSOCKET_URL` | Optional | `src/services/websocket.service.ts` | Direct backend origin for Socket.IO upgrades, e.g. `https://api.example.com`; when unset, realtime uses polling through the Vercel proxy |
...
| `NEXT_PUBLIC_TURNSTILE_ALLOW_TEST_KEYS` | Optional | Turnstile configuration | Set `true` only for temporary hosted testing with Cloudflare test keys; keep `false` or unset when using real keys |
